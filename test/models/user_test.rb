...
class UserTest < ActiveSupport::TestCase
  ...
  test "email addreses should be saved as lower-case" do
    mixed_case_email = "WeAre@Number.One"
  ...
  test "should follow and unfollow a user" do
    paul = users(:paul)
    dan = users(:dan)
    assert_not paul.following?(dan)
    paul.follow(dan)
    assert paul.following?(dan)
    assert dan.followers.include?(paul)
    paul.unfollow(dan)
    assert_not paul.following?(dan)
  end
  ...
  test "feed should have the right posts" do
    ...
    dan.microposts.each do |post_unfollowed|
